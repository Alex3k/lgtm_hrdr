...
output "gke_a_connection_command" {
  value = "gcloud container clusters get-credentials ${var.gcp_gem_gke_cluster_name_a} --region ${var.gcp_region_a} --project ${var.gcp_project_id}"
}

output "gke_b_connection_command" {
  value = "gcloud container clusters get-credentials ${var.gcp_gem_gke_cluster_name_b} --region ${var.gcp_region_b} --project ${var.gcp_project_id}"
}
